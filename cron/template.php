...
# *  *  *  *  * user-name  command to be executed
  *  *  *  *  * root cd && . html-scraper-browserless && export NODE=/usr/local/bin/node && date "+ --- vvv \%Y-\%m-\%d \%H:\%M:\%S vvv ---" >> ./cron/logs/everyminute_$(date +\%Y-\%m-\%d).log  && /bin/bash ./cron/everyminute.sh cron 2>&1 >> ./cron/logs/everyminute_$(date +\%Y-\%m-\%d).log


# to test manually:
cd && . html-scraper-browserless && export NODE=/usr/local/bin/node && date "+ --- vvv %Y-%m-%d %H:%M:%S vvv ---" >> ./cron/logs/everyminute_$(date +%Y-%m-%d).log  && /bin/bash ./cron/everyminute.sh cron 2>&1 >> ./cron/logs/everyminute_$(date +%Y-%m-%d).log
